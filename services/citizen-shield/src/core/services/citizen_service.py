...
class AssessScamRequestDTO(BaseModel):
...
class AssessScamResponseDTO(BaseModel):
    scam_risk: str
    risk_score: float
...
class CitizenService:
    def assess_incident(self, payload: AssessScamRequestDTO) -> AssessScamResponseDTO:
        ...
        guidance = knowledge["SCAM_FOUND"] if risk_score > 0.3 else knowledge["SCAM_NOT_FOUND"]
        steps = knowledge["steps"] if risk_score > 0.3 else ([knowledge["steps"][2]] if len(knowledge["steps"]) > 2 else [])
        
        return AssessScamResponseDTO(
