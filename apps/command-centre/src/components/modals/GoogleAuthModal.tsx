...
import { X, Shield, Lock, ShieldCheck, AlertCircle, ExternalLink } from 'lucide-react';
...
export const GoogleAuthModal: React.FC = () => {
  ...
  const isRawIp = typeof window !== 'undefined' && /^https?:\/\/\d+\.\d+\.\d+\.\d+/.test(window.location.origin);
  const nipUrl = typeof window !== 'undefined'
    ? window.location.href.replace(/http:\/\/(\d+)\.(\d+)\.(\d+)\.(\d+)/, 'http://$1-$2-$3-$4.nip.io')
    : '#';
  ...
  useEffect(() => {
    ...
    const initGoogleGIS = () => {
      if (typeof window !== 'undefined' && (window as any).google?.accounts?.id && !isRawIp) {
        try {
    ...
    initGoogleGIS();
  }, [isGoogleAuthModalOpen, selectedRole, isRawIp]);

  if (!isGoogleAuthModalOpen) return null;

  const handleMobileDirectAuth = () => {
    ...
    const mobileEmail = selectedRole === 'ADMIN' ? 'admin@sentinelx.gov.in' : 'devanshparagsavla1709@gmail.com';
    const mobileName = selectedRole === 'ADMIN' ? 'Insp. R. Sharma (Admin Officer)' : 'DEVANSH SAVLA (Citizen)';

    setBackgroundCheckStatus(`Authenticating Mobile Google Account (${mobileEmail})...`);

    setTimeout(() => {
      loginWithGoogle(mobileEmail, mobileName, selectedRole);
      setBackgroundCheckStatus(`✓ Mobile Google Identity Verified • Clearance Active`);
      setTimeout(() => {
        closeGoogleAuthModal();
        setBackgroundCheckStatus(null);
        setAdminPassword('');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
      <div className="bg-slate-900 border border-slate-700/80 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl space-y-4">
        ...
        <div className="px-8 pb-8 space-y-5">
          ...
          {/* RAW IP NOTICE & NIP.IO SWITCHER FOR MOBILE */}
          {isRawIp && (
            <div className="bg-amber-950/40 p-3.5 rounded-2xl border border-amber-500/40 text-xs space-y-2">
              <div className="flex items-center gap-2 font-bold text-amber-300">
                <AlertCircle className="w-4 h-4 shrink-0" />
                <span>Mobile Wi-Fi IP Detected ({window.location.host})</span>
              </div>
              <p className="text-[11px] text-amber-200/80 leading-relaxed">
                Google OAuth blocks raw IPs with <code className="text-amber-100 font-mono">origin_mismatch</code>. Switch to the authorized domain below or tap <b>Instant Mobile Sign-In</b>.
              </p>
              <a
                href={nipUrl}
                className="w-full py-2 px-3 bg-amber-500/20 hover:bg-amber-500/30 text-amber-200 rounded-xl font-bold text-[11px] flex items-center justify-center gap-1.5 transition-colors border border-amber-500/30"
              >
                <ExternalLink className="w-3.5 h-3.5" />
                <span>Switch to http://10-177-200-94.nip.io:8082</span>
              </a>
            </div>
          )}
          ...
          {/* MANUAL / MOBILE GOOGLE SIGN-IN BUTTON */}
          <button
            type="button"
            onClick={handleManualGoogleClick}
            className="w-full py-3.5 px-4 rounded-xl border border-indigo-500/40 bg-gradient-to-r from-indigo-900/60 to-purple-900/60 hover:from-indigo-800/80 hover:to-purple-800/80 text-white font-bold text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg"
          >
            ...
            <span>{isRawIp ? 'Instant Mobile Google Sign-In' : 'Prompt Google Account Selection'}</span>
          </button>
