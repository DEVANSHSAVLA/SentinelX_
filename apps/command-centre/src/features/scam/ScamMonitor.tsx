...
import { Lock, XCircle, Shield } from 'lucide-react';
import { useData } from '../../context/DataContext';

export const ScamMonitor: React.FC = () => {
  const { sessions, activeSessionId, setActiveSessionId, updateSessionStatus, cases, updateCaseStatus } = useData();

  const activeSessionDetails = sessions.find(s => s.id === activeSessionId) || sessions[0];
  const activeCaseDetails = cases.find(c => c.id === activeSessionId);
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-lg p-6 space-y-4">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="bg-emerald-500/10 text-emerald-400 text-[10px] font-bold px-2 py-0.5 rounded border border-emerald-500/30">
            {sessions.filter(s => s.status !== 'CLOSED').length + cases.filter(c => c.status !== 'CLOSED').length} Active
          </span>
        </div>
        ...
        <div className="space-y-3 max-h-[580px] overflow-y-auto pr-1">
          ...
          {cases.filter(c => c.status !== 'CLOSED').map((rep) => (
            <div
              key={rep.id}
