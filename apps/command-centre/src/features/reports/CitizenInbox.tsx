...
import { FileText, Plus, Search, Filter, Shield, MapPin, FolderCheck, ChevronUp, ChevronDown, Printer, User, ShieldAlert } from 'lucide-react';
import { useData } from '../../context/DataContext';

export const CitizenInbox: React.FC = () => {
  const { userCases, updateCaseStatus, openRegisterCaseModal, currentUser } = useData();
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              {currentUser.role === 'ADMIN'
                ? 'Full Administrative Access: Viewing all public safety incident cases across India.'
                : `Private Citizen Isolation: Viewing and printing only cases registered under ${currentUser.email}.`}
            </p>
          </div>
        </div>

        <div className="text-xs text-slate-400 font-mono">
          Showing: <b className="text-white">{userCases.length}</b> Cases
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl shadow-lg p-6 space-y-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-4 border-b border-slate-800">
          <div>
            <h4 className="font-bold text-xl text-white flex items-center gap-2">
              ...
              Citizen Incident Inbox & Case Ledger
            </h4>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              Print Logged-in User Cases Only
            </button>
        ...
        <div className="flex flex-col md:flex-row gap-3 items-center justify-between bg-slate-850 p-3.5 rounded-lg border border-slate-800">
          ...
          <div className="flex items-center gap-2 w-full md:w-auto justify-end">
            ...
            >
              <option value="ALL">All Active Statuses ({userCases.filter(r => r.status !== 'CLOSED').length})</option>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-sm divide-y divide-slate-800">
              {userCases.filter(r => r.status !== 'CLOSED').length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-8 text-center text-slate-500 text-xs italic">
                    No active cases registered for {currentUser.name}. Click "Register / File New Case" above to file an incident under your account.
                  </td>
                </tr>
              ) : (
                userCases
                  ...
                  .map((rep) => (
                    <tr key={rep.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="py-3 px-4 font-semibold">
                        ...
                        <br/>
                        <span className="text-xs text-slate-400 font-mono font-normal">{rep.reporterEmail}</span>
                      </td>
      ...
      <div className="border border-slate-800 rounded-xl bg-slate-900 shadow-lg overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h5 className="font-bold text-white text-base">Closed Cases Archive ({currentUser.name})</h5>
            <span className="bg-slate-800 text-slate-400 border border-slate-700 text-xs px-2.5 py-0.5 rounded-full font-bold">
              {userCases.filter(r => r.status === 'CLOSED').length} Closed
            </span>
          </div>
